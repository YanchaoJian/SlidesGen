...
├── metrics/                   # LLM-as-Judge 质量评估（手动触发）
├── scripts/                   # 辅助脚本（日志分析、跨基线对比等）
...
| `metrics/pipeline_metrics.py` | `compute_pipeline_metrics()`：基于 `slide_reports` 与 plan 聚合流水线质量指标（首次通过率、自愈率、重试次数等）。 | 手动 |
...
| `scripts/run_eval_comparison.py` | 跨基线批量评估脚本（PPTAgent / Kimi），结果写入 `eval/comparison/`。 |
...
| 路径 | 用途 |
|------|------|
| `data/attention/attention.pdf` | 示例输入论文。 |
| `data/attention/academic.png` | 示例参考样式图。 |
| `data/attention/academic.pptx` | 参考 PPTX（slide master 来源）。 |
