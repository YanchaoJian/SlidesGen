...
from agents.layout_planner.prompts import LAYOUT_DIRECTIVE_SYSTEM_PROMPT, LAYOUT_DIRECTIVE_USER_PROMPT
...
def generate_layout_directive(
    ...
    logger.info(f"🎨 Strategist is planning Slide {slide_page}: '{slide_title}'...")

    # 1. 初始化 LLM
    try:
        llm = create_llm(llm_config, temperature=0.3)
    except Exception as e:
        logger.error(f"❌ Failed to initialize LLM in strategist: {e}")
        return None
