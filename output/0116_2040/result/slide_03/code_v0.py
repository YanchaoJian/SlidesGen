...
def create_presentation(output_pptx_path):
    ...
    # Add bullet points
    text_box = slide.shapes.add_textbox(Inches(0.5), Inches(1.5), Inches(4.5), Inches(3.0))
    text_frame = text_box.text_frame
    text_frame.word_wrap = True
    text_frame.auto_size = MSO_AUTO_SIZE.TEXT_TO_FIT_SHAPE
    text_frame.paragraphs[0].font.size = Pt(16)
    text_frame.paragraphs[0].font.color.rgb = RGBColor(163, 185, 204)
    text_frame.paragraphs[0].font.name = 'Arial'
    text_frame.paragraphs[0].line_spacing = Pt(19.2)  # 1.2 line spacing
    
    bullet_points = [
        "The Transformer model introduces a novel architecture based solely on attention mechanisms.",
        "It eliminates the need for recurrence and convolution, allowing for greater parallelization.",
        "The model sets new benchmarks in machine translation tasks."
    ]
    ...
    for point in bullet_points:
        ...
        p.font.name = 'Arial'
        p.line_spacing = Pt(19.2)
    
    # Add image
    image_path = "path/to/transformer_model_architecture.png"  # Update with actual image path
